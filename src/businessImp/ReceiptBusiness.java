...
import Presentation.Admin.ReceiptPresentation;
...
public class ReceiptBusiness implements IBusiness<Bill, String> {
    ...
    public void displayReceipt(List<Bill> billList) {
        System.out.println("Danh sách thông tin phiếu nhập:");
        billList = findAll();
        drawReceiptTable(billList);
    }

    public void inputReceipt(Scanner scanner) {
        Connection conn = ConnectionDB.openConnection();
        CallableStatement callSt = null;
        System.out.println("Nhập số phiếu nhập muốn thêm:");
            ...
            try {
                if (checkId(input_check) > 0) {
                    ...
                    if (status != 2) {
                        ...
                        billDetailBusiness.drawBillDetailTable(billDetailList);                        callSt = conn.prepareCall("{call get_infor_to_update_bill(?)}");
    ...
    public void confirmReceipt(Scanner scanner, List<Bill> billList) {
        Connection conn = ConnectionDB.openConnection();
        CallableStatement callSt = null;
        CallableStatement callSt2 = null;
        CallableStatement callSt3 = null;
                        ...
                        do {
                            ...
                            switch (choice) {
                                case 1:
                                    ...
                                    if (updateStatus) {
                                        conn.setAutoCommit(false);
                                        callSt3 = conn.prepareCall("{call add_quantity_product(?)}");
    ...
    public void searchReceipt(Scanner scanner, List<Bill> billList) {
        Connection conn = ConnectionDB.openConnection();
        CallableStatement callSt = null;
        CallableStatement callSt2 = null;
        CallableStatement callSt3 = null;
        ...
        if (input_search.length() <= 10) {
            ...
            if (billList != null) {
                displayReceipt(billList);
                System.out.println("Bạn muốn cập nhật hay duyệt phiếu xuất không?\n1. Cập nhật\n2. Duyệt\n3. Thoát");
                do {
                    int choice = ExceptionMethod.validateInteger(scanner);
                    switch (choice) {
                        case 1:
                            System.out.println("Nhập mã phiếu hoặc mã code cần cập nhật từ những phiếu xuất trên:");
                            String input_check = ExceptionMethod.StringNotEmpty(scanner);
                            if (input_check.length() <= 10) {
                                try {
                                    callSt4 = conn.prepareCall("{call check_bill_by_id_bill_or_code_bill(?,?,?)}");
                                    callSt4.setString(1, input_check);
                                    callSt4.setBoolean(2, true);
                        ...
                        case 2:
                            ...
                            if (inputConfirm.length() <= 10) {
                                if (checkId(inputConfirm) > 0) {
                                            ...
                                            do {
                                                ...
                                                switch (choice3) {
                                                    case 1:
                                                        ...
                                                        if (updateStatus) {
                                                            conn.setAutoCommit(false);
                                                            callSt7 = conn.prepareCall("{call add_quantity_product(?)}");
                                    ...
                                    System.err.println("Mã phiếu không tại!");
                                    ReceiptPresentation.menuReceipt(scanner);
                                }
                            } else {
                                System.err.println("Tối đa 10 ký tự!");
    ...
    public void drawReceiptTable(List<Bill> billList) {
        ...
        try {
            ...
            System.out.println("                                            Bảng thông tin phiếu nhập                          ");
            for (int i = 0; i < 136; i++) {
                System.out.print("─");
            }
            System.out.println();
